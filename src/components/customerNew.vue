<template>
  <div class="box">
      ...
      <div class="works">
        <ul class="clearfix">
            <li @click="goHot()">热门作品</li>
            <li class="li-style">最新作品</li>
        </ul>
        <div class="worksPic">
...
export default {
  ...
  methods: {
    ...
    goHot() {
      location.href = "http://localhost:8080/#/customer/1";
    },
    getNew() {
      var url =
        "http://tosneaker.test/api/ext/custom/post?do=new&page=1&limit=10";
      this.$http.get(url).then(res => {
        this.newimgs = res.data.data;
        this.imgsArr = this.initnewImgsArr();
      });
    },
    initnewImgsArr(n, m) {
      var arr = [];
      var imgArr = this.newimgs;
